...
|===
|構成要素 |*パラメータ*
|HTTP connector (HTTP コネクタ) |緑のプラス記号 + をクリックします。
...
|Object Store Connector |緑のプラス記号  + をクリックします。
...
|Set Payload (ペイロードを設定) |*Settings (設定)* = `KEY: &#x0023;[payload.key] AND VALUE: &#x0023;[payload.value] WERE SAVED OK`
|===
+
完了すると、フローが表示されます。
+
